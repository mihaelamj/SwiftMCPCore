...
If you believe you have found a security issue in swift-mcp-core, please report it
...
swift-mcp-core is pre-1.0 and under active development. Security fixes are applied
to the `main` branch. Until a stable release exists, only the latest `main` is
supported.

## Scope

swift-mcp-core is a pure wire-types library (Foundation-only `Codable` value types
